...
begin
  ...
  if length(regexp_replace(p_phone, '\D', '', 'g')) < 7 then
...
revoke all on function public.place_order(text, text, text, public.payment_method, text, jsonb) from public, anon, authenticated;
grant execute on function public.place_order(text, text, text, public.payment_method, text, jsonb) to service_role;
